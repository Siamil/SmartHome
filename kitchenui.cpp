...
void KitchenUI::Initialize(Kitchen *value)
{
    ...
    windowsUI[0]->setAngle(180);
    windowsUI[1]->setAngle(45);
...
void KitchenUI::drawWindows(QPainter *painter)
{
    ...
    windowsUI[0]->draw(painter, this->size(), points[2], points[3]);
    windowsUI[1]->draw(painter, this->size(), points[7], points[6]);
...
{
    heatersUI[0] -> draw(painter, this->size(), points[12]);
}
